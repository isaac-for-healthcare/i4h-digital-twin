...
"""Tests for the piecewise-linear HU to mu transfer function."""

from __future__ import annotations

import numpy as np
import pytest
from vasculature_digital_twin import HuToMuMapping, hu_to_mu, hu_to_mu_curve
...
class TestPreprocessorIntegration:
    ...
    def test_narrower_window_raises_vessel_contrast(self, synthetic_ct_hu, spacing_zyx_mm):
        ...
        assert float(np.ptp(narrow.mu_volume)) > float(np.ptp(wide))
    ...
    def test_with_hu_to_mu_reuses_the_hu_volume(self, synthetic_ct_hu, spacing_zyx_mm):
        ...
        assert rewindowed.settings.hu_to_mu.hu_max == 500.0
    ...
    def test_metadata_records_the_curve(self, synthetic_ct_hu, spacing_zyx_mm):
        ...
        assert HuToMuMapping.from_dict(volume.metadata.hu_to_mu) == mapping
